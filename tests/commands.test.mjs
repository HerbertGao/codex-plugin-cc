...
test("rescue command absorbs continue semantics", () => {
  ...
  // The subagent must always forward to Codex and always run the companion in
  // the foreground. Self-selecting `--background` strands the result (the
  // subagent exits with only a job id / a stub), and answering simple asks
  // itself means Codex never runs — both surface to the user as "the subagent
  // never started". Pin the fixed guidance.
  assert.match(agent, /Once you are invoked, always forward to Codex/i);
  assert.match(agent, /Never answer it yourself/i);
  assert.match(agent, /Always run `task` in the foreground\. Never pass `--background` to the companion/i);
  assert.match(agent, /Never decide to background a task on your own|Backgrounding is decided by the caller/i);
  ...
  assert.match(agent, /If the Bash call fails or Codex cannot be invoked, return the command's stderr/i);
  ...
  assert.match(runtimeSkill, /If the Bash call fails or Codex cannot be invoked, return the command's stderr/i);
